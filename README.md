...
| Path | Maps to |
| --- | --- |
| `.zshrc`, `.zsh/` | `~/.zshrc`, `~/.zsh` — `.zsh/*.zsh` is auto-sourced |
| `.zshrc_light`, `.zsh/zplug_light.zsh` | Slimmer profile for low-power/remote machines (still uses Powerlevel10k) |
| `.p10k.zsh`, `.p10k_console.zsh` | `~/.p10k*` — only used by `.zshrc_light` |
| `.config/` | `~/.config` (a symlink — see the note above) |
...
| Path | What it is |
| --- | --- |
| `nix-darwin/` | **macOS.** The flake: system + Homebrew + home-manager. Declarative, applied with `just switch` |
| `linux/` | **Arch.** `boot/`, `xorg.conf.d/`, `Xsetup` (sddm), `bin/`, `XCompose/` (vendored from [kragen/xcompose]), `xbindkeysrc`, `gitconfig` |
| `windows/` | Windows Terminal settings, `gitconfig` |
| `wsl/` | `zshenv`, `gitconfig` |
| `scripts/` | Cross-platform helpers (`hosts.sh`) |
...
Submodules: `.tmux` and `.config/kitty/kitty-themes` — clone with
