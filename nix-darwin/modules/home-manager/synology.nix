###############################################################################
# Synology DSM (x86_64-linux), shared by every box
#
# Standalone home-manager: a DSM box runs single-user Nix, with neither NixOS
# nor nix-darwin. Its store is a bind mount from a volume. See the Synology
# section of the README. Shares this flake, and so flake.lock, with the MacBook.
#
# Each box has a module of its own that imports this one and adds what only
# that box needs: nas.nix for the RS2423+, bkp.nix for the DS1522+. A box
# activates the profile named after its own hostname; see `nas-switch` in the
# Justfile.
#
...
{ lib, pkgs, ... }:
{
  ...
  # above, so a shell that reads a profile has it.
  #
